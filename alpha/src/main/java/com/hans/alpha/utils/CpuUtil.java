...
import com.hans.alpha.utils.io.FileUtil;
...
public class CpuUtil {
    ...
    private static final String TAG = "MotherShip.CpuUtil";
    ...
    public static String printCpuInfo() {
        StringBuilder info = FileUtil.readFile(CPU_INFO_PATH,"UTF-8");
        if (Log.isPrint) {
    ...
    public static long getCurrentFreqency() {
        try {
            return Long.parseLong(FileUtil.readFile(CPU_FREQ_CUR_PATH,"UTF-8").toString().trim());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return 0;
