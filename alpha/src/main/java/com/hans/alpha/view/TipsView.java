...
import com.hans.alpha.utils.io.PreferencesUtil;
 ...
 * <p>结合使用 {@link ShowTipsBuilder}</p>
 * <url>https://github.com/fredericojssilva/ShowTipsView</url>
 */
public class TipsView extends RelativeLayout {
    ...
    private PreferencesUtil showTipsStore;
    ...
    public void show(final Activity activity) {
        if (isDisplayOneTime() && showTipsStore.getInt(activity, "SHOW_TIPS_ID"+getDisplayOneTimeID()) != -1 ) {
            setVisibility(View.GONE);
            ((ViewGroup) ((Activity) getContext()).getWindow().getDecorView()).removeView(TipsView.this);
            return;
        } else {
            if (isDisplayOneTime())
                showTipsStore.putInt(activity, "SHOW_TIPS_ID"+getDisplayOneTimeID(),getDisplayOneTimeID());
        }
    ...
    private void createViews() {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1) {
            textTitle.setId(generateViewId());
        } else {
